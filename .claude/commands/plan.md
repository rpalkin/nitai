...
**IMPORTANT:** Write the detailed plan directly into `$TASKS_DIR/<id>.md` (NOT into the main repo). Use the `Edit` tool to replace the `## Plan` section with your detailed plan. The `$TASKS_DIR` path is resolved from the environment variable — run `echo $TASKS_DIR` if unsure of the absolute path.
