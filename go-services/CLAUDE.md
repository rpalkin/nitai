...
- **`instructions/`** — `Resolve` (filter `InstructionRow` by repo ID and changed files) + `Merge` (combine API-sourced and YAML-sourced instruction strings). Mirrors `matchInstruction` logic from `api-server/internal/handler/instruction.go`.
...
- **`localmerge/`** — `CreateMergeCommit` utility. Computes a local merge of source branch into target branch using `git merge-tree --write-tree` (plumbing, no working tree). Returns merge commit SHA or `HasConflicts=true` for conflicting PRs.
- **`prreview/`** — `PRReview` Virtual Object. Full orchestrator: smart debounce → DiffFetcher (details + dedup) → draft guard → RepoSyncer → **local merge** (conflicts → exit early with status `conflicts`) → read `.review-rules.yaml` at merge result SHA → resolve org instructions from DB → filter diff by ignore globs → DiffTooLarge check → IndexRepo (target branch + merge result) → Reviewer (with `repo_path`, `merge_sha`, `search_collection`, `custom_instructions`) → PostReview (with `rules_modified` warning). Uses Virtual Object state for debounce timing (`last_started_at`, `last_completed_at`).
...
- **Review statuses** — `review_status` enum: `pending`, `running`, `completed`, `failed`, `skipped` (dedup match), `draft` (MR is a draft), `conflicts` (merge conflicts)
...
- **Local merge before review** — after RepoSyncer fetches all branches, PRReview computes a local merge commit using `git merge-tree --write-tree`. This merge result SHA is used for file reads and search indexing, ensuring the reviewer sees the exact post-merge state. If the merge fails (conflicts), the run is marked `conflicts` and exits early.
- **PRReview.Run pipeline** — debounce → fetch → dedup → draft guard → SyncRepo → **local merge** (conflicts → exit) → read `.review-rules.yaml` at merge SHA → fetch org instructions from DB → filter diff → DiffTooLarge check → IndexRepo (target + merge result) → resolve + merge instructions → Reviewer (with `repo_path`, `merge_sha`, `search_collection`, `custom_instructions`) → PostReview (with `rules_modified` warning). SyncRepo failure is fatal; IndexRepo failure is non-fatal (review proceeds without search). Rules reading and org instruction resolution are non-fatal (proceeds without instructions on failure).
